...
export class SearchBar extends HTMLElement {
 ...
 <div
    ...
    connectedCallback() {
        ...
        if (placeholderAttr === null) throw new Error("not seeing placeholder passed to searchBar")
        if (placeholderAttr === "") {
            placeholderAttr = "Search"
        }

        const idAttr = this.getAttribute('id')
        if (idAttr === null) throw new Error("not seeing id passed to searchBar")

        //add on placeholder
        const seenEl = getElement<HTMLInputElement>("input", undefined, this)
        seenEl.placeholder = placeholderAttr
        seenEl.id = idAttr
    }
}

export interface SearchBar extends HTMLElement {
    value: string;
    options: string
}
