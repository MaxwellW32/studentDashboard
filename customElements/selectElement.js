...
export class CustomSelect extends HTMLElement {
    ...
    constructor() {
        super();
        const template = getElement("#select-element-template").content;
        // add template
    ...
    connectedCallback() {
        ...
        if (optionsAttr === null)
            throw new Error("not seeing optionsAttr passed to select element");
        const options = optionsAttr.split(',');
        const valueAttr = this.getAttribute('value');
        if (valueAttr === null)
            throw new Error("not seeing valueAttr passed to select element");
        ...
        if (options[0] === "||datePicker||") {
            ...
            const dateSelectionInput = getElement(".dateSelectionInput", undefined, this);
