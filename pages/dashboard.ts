...
function dashboard() {
    ...
    const scoreDateSelection = getElement<CustomSelectElement>("custom-select#scoreDateSelection");
    const subjectDateSelection = getElement<CustomSelectElement>("custom-select#subjectDateSelection");
    const subjectGradeSelection = getElement<CustomSelectElement>("custom-select#subjectGradeSelection");

    //respond to selection changes
    scoreDateSelection.addEventListener('change', (event: any) => {
        console.log('New value:', event.detail.value);
    });
    subjectDateSelection.addEventListener('change', (event: any) => {
        console.log('New value:', event.detail.value);
    });
    subjectGradeSelection.addEventListener('change', (event: any) => {
        console.log('New value:', event.detail.value);
    });
    ...
    function changeSelectedDate(numberOfMonths: number) {
...
function generateTable() {
    const table = getElement<HTMLTableElement>("#dashboardAssignmentsTable");
    table.innerHTML = ""; // clear previous fee collection table

    //make a new row
    const tableHeadingRowTemplate = getElement<HTMLTemplateElement>("#dashboardAssignmentsTableHeadingRowTemplate")
    ...
    dummyData.assignments.forEach((eachAssignment, eachAssignmentIndex) => {
        const tableRowTemplate = getElement<HTMLTemplateElement>("#dashboardAssignmentsTableRowTemplate")
        const tableRowClone = tableRowTemplate.content.cloneNode(true) as HTMLTableRowElement;

        const dashboardAssignmentRecordNoEl = getElement<HTMLImageElement>(".dashboardAssignmentRecordNo", undefined, tableRowClone)
...
function generateGradeTable() {
    ...
    exampleSubjectGrades.forEach(eachSubjectGrade => {
        const gradeSubjectTemplateEl = getElement<HTMLTemplateElement>("#gradeSubjectTemplate")
        const gradeSubjectTemplateClone = gradeSubjectTemplateEl.content.cloneNode(true) as HTMLTableRowElement;
